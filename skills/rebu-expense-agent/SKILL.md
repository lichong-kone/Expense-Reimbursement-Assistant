...
9. **产不出就停下并如实告知，绝不自行替代**：若某目标文件当前 Skill/MCP 没有对应工具，**停止并明确告诉用户"暂不支持"**，不得自行生成替代版本。
   - **《出差申请表》：当前无 MCP 导出工具，且政策 skill 明确"先跳过"。** 被要求同时出两份时，只导出《费用报销单》一份；第二份明确回复"暂不支持——需后端新增《出差申请表》导出工具及公司官方模板后才能生成"，不得用 Python/手工造表替代。
...
- **只有《费用报销单》可导出。** 若用户还要《出差申请表》：明确回复"暂不支持"（无 MCP 工具、政策 skill 先跳过），**不得**用 Python/手工造表替代（见铁律 8/9）。
- 交付时附带：
  - 文件名与大小
  - 报销单总额 / 可报销额
